...
**Architecture:** The existing `wellKnownJSONForms` table remains the single declaration-site discriminator for well-known messages. A focused emitter module maps its Struct/List/Value and Timestamp/Duration forms to additional `fsast.Func` members; ordinary messages, `Any`, and wrappers receive none. Native conversion failures use appended `ProtobufError` cases and tuple results, matching the generated wire factories, while the existing JSON surface keeps its `JsonResult[T]` contracts.
...
- Modify `internal/runtime/data/foundry/proto/protobuf_error.fs` and `internal/runtime/runtime_test.go`: append and pin `STRUCT_KEY_NOT_STRING` and `STRUCT_VALUE_UNREPRESENTABLE` without renumbering existing cases.
- Modify `tests/foundry/main.fs`: exercise native recursion/rejections and time normalization/precision against alpha19.
...
- [ ] Add runtime tests for the two appended error numbers and for a narrow public-Variant exception covering only the two exact Value bridge signatures.
...
- [ ] Emit strict inbound methods. Use `typeof` rather than `is` for builtins, `float(int_value)` for the documented narrowing, typed output containers, and local construction so a failing branch returns no partial object.
- [ ] Restrict `CheckPublicAPI`'s exception to exact whole-line spellings of `to_variant`, `from_variant`, `to_dictionary`, `to_array`, and `from_array` shown in Task 1; keep every other public Variant signature rejected.
...
- [ ] Add `check_well_known_time_helpers()` before production changes. Cover Timestamp `1700000000.5`, `-0.25`, and a fractional value whose rounded nanos carry; cover Duration `1.25`, `-1.25`, sub-second negative values, and rounded positive/negative carries. Assert Timestamp nanos are `[0, 999999999]`, Duration seconds/nanos have compatible signs, and `now()` is near `Time.get_unix_time_from_system()`.
- [ ] Document and assert float precision by choosing a unix float, converting into fields and back, and comparing the returned float exactly to the representable input; separately show that manually supplied nanoseconds can round when converted out.
- [ ] Run `task foundry:test`; expect lint failure because `Timestamp.from_unix_time` is absent.
- [ ] Emit type-safe `floori`/`roundi` Timestamp construction with positive carry and canonical nonnegative nanos. Emit truncation-toward-zero Duration construction with `roundi` and both carry directions. Both outbound methods return `float(seconds) + float(nanos) / 1000000000.0`; `now()` delegates to `from_unix_time(Time.get_unix_time_from_system())`.
...
- [ ] Update README with method names, strict rejection cases, int-to-float loss beyond 2^53, Timestamp's canonical negative representation, Duration sign normalization, and outbound float precision loss. State that `Any` remains unsupported and wrappers remain ordinary messages.
