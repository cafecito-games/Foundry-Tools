...
static func from_bytes(_pb_data: PackedByteArray) -> (FloatValue?, ProtobufError):
	...
	if _pb_error != ProtobufError.OK:
		var _pb_failed: FloatValue? = null
...
func to_bytes() -> PackedByteArray:
	...
	if not Wire.is_default_float32(value):
		_pb_result.append_array(Wire.encode_varint(Wire.make_tag(1, Wire.WIRE_32BIT)))
		_pb_result.append_array(Wire.encode_float(value))
	...
	while _pb_offset < _pb_data.size():
		...
		match Wire.get_field_number(_pb_tag.value):
			1:
				...
				var _pb_value_read: FloatRead = Wire.read_float(_pb_data, _pb_offset)
