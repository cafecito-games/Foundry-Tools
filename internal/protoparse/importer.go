...
// duplicates.
func ResolveExternalWithFiles(file *protoast.ProtoFile, inputPath string, fs FS) ([]ImportedFile, error) {
	_ = inputPath
...
func collectFromMessage(out map[string]importedType, m *protoast.Message, prefix, relativePrefix, sourceFile string) {
...
func annotateMessage(m *protoast.Message, lookup map[string]importedType) {
