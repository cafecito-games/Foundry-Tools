...
// NewCommand builds the package-manager command tree.
func NewCommand(stdout, stderr io.Writer) *cobra.Command {
	cmd := newCommand(&packageCLIOptions{})
	cmd.SetOut(stdout)
	cmd.SetErr(stderr)
	return cmd
}

func newCommand(opts *packageCLIOptions) *cobra.Command {
	if opts == nil {
...
func newPkgInitCommand(opts *packageCLIOptions) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, _ []string) error {
			result, err := packageInit(InitOptions{Dir: dir})
			if err != nil {
				return err
			}
...
func newPkgAddCommand(opts *packageCLIOptions) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, _ []string) error {
			spec := PackageSpec{
				Name:       flags.name,
				Source:     SourceType(flags.source),
				URL:        flags.url,
				Repo:       flags.repo,
			...
			results, err := packageAdd(cmd.Context(), AddOptions{
...
func newPkgInstallCommand(opts *packageCLIOptions) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			results, err := packageInstall(cmd.Context(), InstallOptions{
...
func newPkgUpdateCommand(opts *packageCLIOptions) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			results, err := packageUpdate(cmd.Context(), UpdateOptions{
				Options: packageManagerOptions(dir, opts),
				Names:   args,
			})
			if err != nil {
...
func newPkgRemoveCommand(opts *packageCLIOptions) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			name := args[0]
			if err := packageRemove(RemoveOptions{
				Options: packageManagerOptions(dir, opts),
				Name:    name,
			}); err != nil {
				return err
...
func newPkgListCommand(opts *packageCLIOptions) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Args:  cobra.NoArgs,
		RunE: func(cmd *cobra.Command, _ []string) error {
			listings, err := packageList(ListOptions{
				Options: packageManagerOptions(dir, opts),
			})
			if err != nil {
				return err
...
func packageManagerOptions(dir string, opts *packageCLIOptions) Options {
	return Options{
		Dir:               dir,
