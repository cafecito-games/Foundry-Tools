...
// FetchResult is the outcome of fetching a package source into a temp directory.
...
// Fetcher retrieves a package source into a local temporary directory.
// Callers are responsible for removing FetchResult.Dir when done.
type Fetcher interface {
	Fetch(ctx context.Context, spec manifest.PackageSpec) (FetchResult, error)
}
...
func FetcherFor(spec manifest.PackageSpec) (Fetcher, error) {
...
func FetcherForWithLimits(limits Limits) func(manifest.PackageSpec) (Fetcher, error) {
	return func(spec manifest.PackageSpec) (Fetcher, error) {
		switch spec.Source {
