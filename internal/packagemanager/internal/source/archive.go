...
func (f *ArchiveFetcher) Fetch(ctx context.Context, spec manifest.PackageSpec) (FetchResult, error) {
	...
	dir, err := os.MkdirTemp("", "anvil-archive-*")
	if err != nil {
...
func downloadToFile(ctx context.Context, client *http.Client, rawURL string, header http.Header, maxBytes int64) (string, string, error) {
	...
	tmp, err := os.CreateTemp("", "anvil-download-*")
	if err != nil {
