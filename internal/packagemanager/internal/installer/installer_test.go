...
func TestInstallExplicitSourcePath(t *testing.T) {
	...
	err := Install(source.FetchResult{Dir: fetched}, manifest.PackageSpec{
...
func TestInstallAutoDetectsSingleAddonsDir(t *testing.T) {
	...
	addonsDir := t.TempDir()
	require.NoError(t, Install(source.FetchResult{Dir: fetched}, manifest.PackageSpec{Name: "dlg"}, addonsDir))
...
func TestInstallAmbiguousFails(t *testing.T) {
	...
	err := Install(source.FetchResult{Dir: fetched}, manifest.PackageSpec{Name: "dlg"}, t.TempDir())
	require.Error(t, err)
...
func TestInstallRejectsEscapingInstallName(t *testing.T) {
	...
	spec := manifest.PackageSpec{Name: "dlg", InstallAs: ".."}
...
func TestInstallRollbackOnCopyFailure(t *testing.T) {
	...
	require.NoError(t, Install(source.FetchResult{Dir: firstFetched}, manifest.PackageSpec{Name: "dlg"}, addonsDir))
	...
	err = Install(source.FetchResult{Dir: badFetched}, manifest.PackageSpec{Name: "dlg"}, addonsDir)
	require.Error(t, err)
	var installErr *output.InstallError
	require.ErrorAs(t, err, &installErr)
...
func TestInstallRejectsSymlink(t *testing.T) {
	...
	err := Install(source.FetchResult{Dir: fetched}, manifest.PackageSpec{Name: "dlg"}, addonsDir)
	require.Error(t, err)
	var installErr *output.InstallError
	require.ErrorAs(t, err, &installErr)
}
...
func TestInstallRecoversInterruptedBackup(t *testing.T) {
	...
	// .anvil-backup copy survives.
	writeTree(t, filepath.Join(addonsDir, "dlg.anvil-backup"), map[string]string{"plugin.cfg": "old"})
	...
	_, err = os.Stat(filepath.Join(addonsDir, "dlg.anvil-backup"))
...
func TestInstallDiscardsStaleBackupWhenDestinationPresent(t *testing.T) {
	...
	writeTree(t, filepath.Join(addonsDir, "dlg.anvil-backup"), map[string]string{"plugin.cfg": "stale"})
