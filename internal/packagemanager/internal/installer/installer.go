...
// If copying fails partway, the existing package is left untouched (rollback-safe).
func Install(fetched source.FetchResult, spec manifest.PackageSpec, addonsDir string) error {
	...
	backupPath := destination + ".anvil-backup"
	...
	staging, err := os.MkdirTemp(addonsDir, ".anvil-staging-*")
	if err != nil {
		return &output.InstallError{Err: err}
	}
	...
	if destinationExists {
		if err := os.Rename(destination, backupPath); err != nil {
			_ = os.RemoveAll(staging)
			return &output.InstallError{Err: fmt.Errorf("could not back up existing package: %w", err)}
		}
		if err := os.Rename(staging, destination); err != nil {
			// Restore the backup so the package is not lost.
			_ = os.Rename(backupPath, destination)
			_ = os.RemoveAll(staging)
			return &output.InstallError{Err: fmt.Errorf("could not move staged package into place: %w", err)}
		}
...
// leftover `<destination>.anvil-backup` exists, it is restored when destination
...
func recoverBackup(destination, backupPath string) error {
	...
	if _, err := os.Stat(destination); errors.Is(err, fs.ErrNotExist) {
		if err := os.Rename(backupPath, destination); err != nil {
			return &output.InstallError{Err: fmt.Errorf("restoring interrupted package backup: %w", err)}
		}
		return nil
	}
...
func copyTree(src, dst string, excludeDirs []string) error {
	...
	return filepath.WalkDir(src, func(path string, entry os.DirEntry, err error) error {
		...
		if entry.Type()&os.ModeSymlink != 0 {
			return &output.InstallError{Err: fmt.Errorf(
				"package source contains an unsupported symlink: %s", entry.Name())}
		}
