...
func validate(t *testing.T, src string) []protovalidate.ValidationError {
	t.Helper()
	tokens, err := protoparse.Tokenize(src, "test.proto")
	if err != nil {
		t.Fatalf("lex: %v", err)
	}
	file, err := protoparse.Parse(tokens, "test.proto")
	if err != nil {
		t.Fatalf("parse: %v", err)
	}
	return protovalidate.Validate(file, "test.proto")
}

func TestValidationErrorFormat(t *testing.T) {
	e := &protovalidate.ValidationError{File: "x.proto", Line: 5, Column: 10, Message: "boom"}
...
func TestValidationErrorDefaultFile(t *testing.T) {
	e := &protovalidate.ValidationError{Line: 1, Column: 1, Message: "oops"}
...
message Drawing {
	...
	errs := protovalidate.Validate(file, "in.proto")
	if len(errs) != 0 {
...
func TestImportedFieldSkipsLocalResolution(t *testing.T) {
	importedAST := &protoast.ProtoFile{
		Position: protoast.Position{Line: 1, Column: 1},
		Syntax:   "proto3",
		Messages: []*protoast.Message{
			{
				Position: protoast.Position{Line: 2, Column: 1},
				Name:     "F",
	...
	errs := protovalidate.Validate(importedAST, "test.proto")
	if len(errs) != 0 {
