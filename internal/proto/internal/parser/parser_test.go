...
func parseSource(t *testing.T, src string) (*protoast.ProtoFile, error) {
	t.Helper()
	tokens, err := protoparse.Tokenize(src, "test.proto")
	if err != nil {
		t.Fatalf("lex error: %v", err)
	}
	return protoparse.Parse(tokens, "test.proto")
}

func TestParserErrorFormat(t *testing.T) {
	tok := protoparse.Token{Type: protoparse.TokenIdentifier, Value: "Foo", Line: 3, Column: 7}
	err := &protoparse.ParserError{File: "x.proto", Token: tok, Message: "boom"}
...
func TestParserErrorDefaultFile(t *testing.T) {
	err := &protoparse.ParserError{Token: protoparse.Token{Line: 1, Column: 1}, Message: "oops"}
...
func TestReservedNumbers(t *testing.T) {
	...
	want := []protoast.ReservedRange{{Start: 2, End: 2}, {Start: 15, End: 15}, {Start: 9, End: 9}}
...
func TestReservedRanges(t *testing.T) {
	...
	r := file.Messages[0].Reserved[0]
	want := []protoast.ReservedRange{{Start: 2, End: 2}, {Start: 9, End: 11}, {Start: 15, End: 15}}
	for i, w := range want {
		if r.Numbers[i] != w {
...
func TestParseFoundryFileOptions(t *testing.T) {
	src := `syntax = "proto3";
import "foundrytools/options.proto";
option (foundrytools.namespace) = "gameplay.inventory";
...
`
	...
	cases := map[string]any{
		"(foundrytools.namespace)":    "gameplay.inventory",
		"(foundrytools.type_prefix)":  "Game",
		"(foundrytools.emit_runtime)": true,
	}
	for key, want := range cases {
		got, ok := file.Options[key]
		if !ok {
			t.Fatalf("option key %s not set", key)
		}
		if got != want {
			t.Fatalf("%s = %v, want %v", key, got, want)
		}
		pos, ok := file.OptionPositions[key]
		if !ok {
			t.Fatalf("OptionPositions missing entry for %s", key)
		}
		if pos.Line == 0 || pos.Column == 0 {
			t.Fatalf("expected non-zero position for %s, got %+v", key, pos)
		}
	}
}
