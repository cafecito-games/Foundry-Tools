...
func TestConvertDeclarationPositionsFromSourceCodeInfo(t *testing.T) {
	fdp := &descriptorpb.FileDescriptorProto{
		...
		MessageType: []*descriptorpb.DescriptorProto{
			{
				...
				Field: []*descriptorpb.FieldDescriptorProto{
					...
					},
					{
						...
						Number:   i32Ptr(2),
						Type:     typePtr(descriptorpb.FieldDescriptorProto_TYPE_MESSAGE),
						TypeName: strPtr(".Outer.LabelsEntry"),
						Label:    labelPtr(descriptorpb.FieldDescriptorProto_LABEL_REPEATED),
					},
				},
			},
		...
		SourceCodeInfo: &descriptorpb.SourceCodeInfo{
			Location: []*descriptorpb.SourceCodeInfo_Location{
				...
				{Path: []int32{4, 0, 2, 0}, Span: []int32{12, 3, 12, 18}},
				{Path: []int32{4, 0, 2, 1}, Span: []int32{13, 3, 13, 24}},
	...
	require.Equal(t, protoast.Position{Line: 13, Column: 4}, got.Messages[0].Oneofs[0].Fields[0].Position)
	require.Equal(t, protoast.Position{Line: 14, Column: 4}, got.Messages[0].Maps[0].Position)
...
func TestConvertFilePropagatesFoundryOptions(t *testing.T) {
	opts := &descriptorpb.FileOptions{}
	proto.SetExtension(opts, foundrytoolspb.E_Namespace, "gameplay.inventory")
	proto.SetExtension(opts, foundrytoolspb.E_TypePrefix, "Game")
	proto.SetExtension(opts, foundrytoolspb.E_EmitRuntime, true)
...
func TestConvertFileMissingFoundryOptions(t *testing.T) {
	fd := &descriptorpb.FileDescriptorProto{
		Name:   strPtr("hero.proto"),
		Syntax: strPtr("proto3"),
	}
	...
	for _, key := range []string{
		"(foundrytools.namespace)",
		"(foundrytools.type_prefix)",
		"(foundrytools.emit_runtime)",
	} {
		if _, ok := file.Options[key]; ok {
			t.Fatalf("option key %s should be absent", key)
		}
	}
}
